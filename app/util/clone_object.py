...
def clone_object(lang_name, lang_code, spacy_language):
    ...
    if (
        new_lang_path.exists()
    ):  # TODO Revisit this choice, should we replace if exists? Users might not know this and delete an existing project.
    ...
    spacy_lang_path = (
        Path(spacy.__file__.replace("__init__.py", "")) / "lang" / spacy_code
    )
    assert (
        spacy_lang_path.exists()
    ), "spaCy lang directory not found, please install spaCy or check version"

    # copy files from spacy.lang to new_lang directory
    core_files = [
        x for x in spacy_lang_path.glob("**/*") if x.is_file() and "pyc" not in str(x)
    ]
    ...
    new_files = [
        x for x in new_lang_path.glob("**/*") if x.is_file() and "pyc" not in str(x)
    ]
    for file in new_files:
        ...
        file_text = file_text.replace(
            '"' + spacy_code + '"', '"' + new_lang_code + '"'
        )  # quotes added to avoid false matches
    ...
    assert (
        spacy_lookups.exists()
    ), "Spacy lookups  data not installed, please run pip install spacy[lookups] or pip install spacy-lookups-data"

    new_lookups = new_lang_path / "lookups-data"
    new_lookups.mkdir(parents=True, exist_ok=True)
    lookups_files = [
        x
        for x in spacy_lookups.glob("**/*")
        if x.is_file() and spacy_code + "_" in str(x)
    ]
